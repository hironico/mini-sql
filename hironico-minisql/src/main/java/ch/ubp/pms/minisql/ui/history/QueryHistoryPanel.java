...
import ch.ubp.pms.minisql.ui.CopyAction;
...
public class QueryHistoryPanel extends JPanel {
    ...
    protected JButton btnToEditor = null;
    ...
    private JXTextField getTxtFilter() {
        if (this.txtFilter == null) {
            ...
            txtFilter.addActionListener(evt -> {
                getTableHistory().applyFilter(txtFilter.getText(), 1);
            });
        }
        ...
        if (tableHistory == null) {
            ...
            tableHistory.addMouseMotionListener(new MouseMotionAdapter() {
                ...
                public void mouseMoved(MouseEvent e) {
                    ...
                    int length = value.length() > 300 ? 300 : value.length();
                    Date time = new Date(entry.timestamp);
                    String queryText = value.substring(0, length).replaceAll("\n", "<br/>");
                    String html = String.format("<html><h4>Executed at %s</h4><code style=\"background-color: white;\">%s</code></html>", time.toString(), queryText);
            ...
            tableHistory.getSelectionModel().addListSelectionListener(evt -> {
                ...
                getLblInfos().setText(entry == null ? "" : new Date(entry.timestamp).toLocaleString());
            });
